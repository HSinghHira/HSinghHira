...
export function remarkLeafDirectives() {
  return tree => {
    // Handle leaf directives (block-level embeds)
    ...
    // Restore unrecognized text directives back to plain text.
    //
    // remark-directive parses any `:word` sequence as a textDirective, which
    // breaks patterns like:
    //   @hsinghhira:matrix.org  →  text("@hsinghhira") + textDirective{name:"matrix", class:"org"}
    //   15:30                   →  text("15")           + textDirective{name:"30"}
    //
    // We reconstruct the original raw text by restoring `:name` and then
    // re-appending any shorthand attributes (.class → ".org", #id → "#foo")
    // that remark-directive extracted from the source.
    visit(tree, "textDirective", node => {
      // The colon is the actual directive trigger — restore it
      let raw = `:${node.name}`;

      // Re-append shorthand attribute syntax that was parsed out
      const attrs = node.attributes ?? {};
      if (attrs.id) raw += `#${attrs.id}`;
      if (attrs.class) raw += `.${attrs.class.split(" ").join(".")}`;

      // Re-append label text from children (e.g. :name[label])
